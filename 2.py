...
for i in range(len(close_prices)):
    if close_prices[i] > sma[i] + sd[i] and position == 0:
        ...
        units = capital / 3  # Enter with 1/3 of the capital
        position = -1  # Negative value indicates a sell position
    elif close_prices[i] > sma[i] + 2 * sd[i] and position == -1:
        # Scale-in the sell position by another 1/3 of the capital
        additional_units = min(capital / 3, (capital - units * close_prices[i]) / 2)
        units += additional_units
    elif close_prices[i] > sma[i] + 3 * sd[i] and position == -1:
        # Scale-in the sell position by the remaining available capital
        additional_units = capital - units * close_prices[i]
        units += additional_units
        ...
        # Scale-in the buy position by another 1/3 of the capital
        additional_units = min(capital / 3, (capital - units * close_prices[i]) / 2)
        units += additional_units
    elif close_prices[i] < sma[i] - 3 * sd[i] and position == 1:
        # Scale-in the buy position by the remaining available capital
        additional_units = capital - units * close_prices[i]
        units += additional_units
